...
const CartItemContainer: React.FC<CartItemContainerProps> = ({
  ...
  const handleRemove = (instanceId: string) => {
    setAddedProducts((prev) => prev.filter((product) => product.instanceId !== instanceId));
  };
  ...
  return (
    ...
    >
      {addedProducts.map((product) => (
        <li key={product.instanceId} className={`rounded-lg shadow-md flex items-center justify-between py-4 px-3 ${
